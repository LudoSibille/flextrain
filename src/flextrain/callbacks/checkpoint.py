...
class CallbackCheckpoint(L.Callback):
    def __init__(
        ...
        save_last: bool = True,
        save_model_check_fn: Optional[
            Callable[["CallbackCheckpoint", L.Trainer, L.LightningModule], bool]
        ] = save_model_no_NaN,
        ...
        if self.metric_name is not None:
            assert (
                metric_to_maximize is not None
            ), f'metric={metric_name} is defined. Is this to be maximized or minimized? This must be specified!'
    ...
    def on_train_epoch_end(self, trainer: L.Trainer, pl_module: L.LightningModule):
        ...
        if self.save_model_check_fn is not None:
        ...
        if len(self.last_checkpoints) >= self.save_top_k:
            ...
            if self.metric_name is not None:
                if metric_value is None:
                    logger.info(
                        f'metric={self.metric_name} not found in the metrics. Available={list(trainer.callback_metrics.keys())}. Top-k not recorded!'
                    )
                    return
        ...
        if metric_value is not None:
            path_checkpoint = os.path.join(
                path_root, f'checkpoint_{trainer.current_epoch}_{self.metric_name}_{metric_value}.pth'
            )
        else:
