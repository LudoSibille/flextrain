...
from .kdiff_unet_v1_layers import AdaGN, Downsample2d, CrossAttention2d, ConditionedSequential, Upsample2d, FourierFeatures, UNet, ConditionedResidualBlock, SelfAttention2d
from .kdiff_unet_v1_layers import append_dims
...
class DBlock(ConditionedSequential):
    def __init__(self, n_layers, feats_in, c_in, c_mid, c_out, group_size=32, head_size=64, dropout_rate=0., downsample=False, self_attn=False, cross_attn=False, c_enc=0):
        ...
        for i in range(n_layers):
            ...
            if cross_attn:
                norm = lambda c_in: AdaGN(feats_in, c_in, max(1, my_c_out // group_size))
                modules.append(CrossAttention2d(my_c_out, c_enc, max(1, my_c_out // head_size), norm, dropout_rate))
...
class ImageDenoiserModelV1(nn.Module):
    def __init__(self, c_in, feats_in, depths, channels, self_attn_depths, cross_attn_depths=None, mapping_cond_dim=0, unet_cond_dim=0, cross_cond_dim=0, dropout_rate=0., patch_size=1, skip_stages=0, has_variance=False):
        ...
        self.u_net = UNet(d_blocks, reversed(u_blocks), skip_stages=skip_stages)
